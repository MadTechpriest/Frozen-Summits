		...
		if(health < maxHealth)
			...
			if(src != M)
				...
				M.visible_message(span_danger("[M] repairs some of \the <b>[src]'s</b> dents."), \
						   span_cult("I repair some of <b>[src]'s</b> dents, leaving <b>[src]</b> at <b>[health]/[maxHealth]</b> health."))
			else
				M.visible_message(span_danger("[M] repairs some of [p_their()] own dents."), \
						   span_cult("I repair some of your own dents, leaving you at <b>[M.health]/[M.maxHealth]</b> health."))
		else
			if(src != M)
				to_chat(M, span_cult("I cannot repair <b>[src]'s</b> dents, as [p_they()] [p_have()] none!"))
			else
	...
	if(istype(P, /obj/projectile/energy) || istype(P, /obj/projectile/beam))
		...
		if(prob(reflectchance))
			...
			visible_message(span_danger("The [P.name] is reflected by [src]'s armored shell!"), \
							span_danger("The [P.name] is reflected by your armored shell!"))
